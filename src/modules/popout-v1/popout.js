...
.pop-msg-actions{margin-left:auto;display:flex;gap:4px;opacity:.72;transition:opacity .15s ease}.cm:hover .pop-msg-actions{opacity:1}.pop-msg-act-btn{border:1px solid #3a4d68;border-radius:5px;background:#1c2330;color:#a8b5c8;font-size:10px;line-height:1;padding:4px 6px;cursor:pointer}.pop-msg-act-btn:hover{color:#e4e9f0;border-color:#4a7ff5}.pop-msg-act-btn.danger:hover{color:#f0aaaa;border-color:rgba(220,90,90,.55)}
...
.mt{font-size:14.5px;color:#a8b5c8;line-height:1.65;word-break:break-word}
...
L.push('window.renderDmBar=renderDmBar;');
  ...
  L.push('function attachPopMessageActions(row,type,meta,text,ch){if(!row||!meta)return;var key=String(meta.key||"").trim(),uid=String(meta.uid||"").trim();if(!key||type==="system"||type==="sys")return;var mine=uid&&uid===getMyUid(),canDelete=mine||popIsGM,canEdit=mine&&type!=="dice"&&type!=="image"&&type!=="speak-as-image";if(!canEdit&&!canDelete)return;var head=row.querySelector(".mn")||row.querySelector(".msg-meta")||row.querySelector(".mb");if(!head)return;var wrap=document.createElement("span");wrap.className="pop-msg-actions";if(canEdit){var eb=document.createElement("button");eb.type="button";eb.className="pop-msg-act-btn";eb.textContent="수정";eb.onclick=function(ev){ev.preventDefault();ev.stopPropagation();var next=window.prompt("메시지를 수정하세요.",String(text||""));if(next===null)return;next=String(next).trim();if(!next){window.alert("빈 메시지는 입력할 수 없어요.");return}var op=getOpenerState();if(op&&typeof op.updateChatMessageFromPopout==="function"){Promise.resolve(op.updateChatMessageFromPopout(key,ch||"chat",next)).then(function(ok){if(!ok)return;try{if(op.forcePopoutSync)op.forcePopoutSync()}catch(e){}})}else if(op&&typeof op.editChatMessageFromPopout==="function"){Promise.resolve(op.editChatMessageFromPopout(key,ch||"chat")).then(function(){try{if(op.forcePopoutSync)op.forcePopoutSync()}catch(e){}})}else{window.alert("메인 창과 연결되지 않아 수정할 수 없어요.")}};wrap.appendChild(eb)}if(canDelete){var dbtn=document.createElement("button");dbtn.type="button";dbtn.className="pop-msg-act-btn danger";dbtn.textContent="삭제";dbtn.onclick=function(ev){ev.stopPropagation();var op=getOpenerState();if(op&&typeof op.deleteChatMessageFromPopout==="function")Promise.resolve(op.deleteChatMessageFromPopout(key,ch||"chat")).then(function(){try{if(op.forcePopoutSync)op.forcePopoutSync()}catch(e){}})};wrap.appendChild(dbtn)}head.appendChild(wrap)}');
