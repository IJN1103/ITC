...
function buildPopoutScript(journalJson, playersJson) {
  ...
  L.push('function renderDmBar(){var host=document.getElementById("pdm");if(!host)return;var op=getOpenerState();if(!op||typeof op.getCurrentDmChannelKey!=="function"){host.innerHTML="";return}var currentKey=op.getCurrentDmChannelKey()||"global";var html=[];function dotHtml(){return "<span style=\"position:absolute;top:4px;right:4px;width:6px;height:6px;border-radius:999px;background:#b89a60\"></span>"}function hasUnreadForPlayer(uid){try{if(!op.getAvailableDmChannels||!op.getDmUnreadState)return false;var chans=op.getAvailableDmChannels()||[];return chans.some(function(ch){return Array.isArray(ch&&ch.participantIds)&&ch.participantIds.indexOf(uid)>=0&&op.getDmUnreadState(ch.channelKey)})}catch(e){return false}}html.push(`<button type="button" class="pdm-btn ${currentKey==="global"?"on":""}" data-role="global"><span class="pdm-label">전체</span></button>`);try{if(op.isDmGmView&&op.isDmGmView()){var players=((op.St&&op.St.players)||{});Object.keys(players).forEach(function(uid){var p=players[uid]||{};if(String(uid)===String((op.St&&op.St.myId)||""))return;if(String(p.role||"").toLowerCase()==="gm")return;var selected=(op.__DM_CHANNEL_STATE&&Array.isArray(op.__DM_CHANNEL_STATE.selectedParticipantIds)?op.__DM_CHANNEL_STATE.selectedParticipantIds:[]).indexOf(uid)>=0;var alias=(typeof op.getDmButtonAlias==="function"?op.getDmButtonAlias(uid):"")||"";var label=alias||String(p.name||"플레이어");var dot=hasUnreadForPlayer(uid)?dotHtml():"";html.push(`<button type="button" class="pdm-btn ${(selected&&currentKey!=="global")?"on":""}" data-role="player" data-uid="${escHtml(uid)}"><span class="pdm-label">${escHtml(label)}</span>${dot}</button>`)})}else if(op.getPlayerVisibleDmChannels){var chans=op.getPlayerVisibleDmChannels((op.St&&op.St.myId)||"")||[];chans.forEach(function(ch){var ids=Array.isArray(ch.participantIds)?ch.participantIds:[];var me=String((op.St&&op.St.myId)||"");var players=((op.St&&op.St.players)||{});var label=ids.filter(function(uid){return String(uid)!==me}).map(function(uid){return String((players[uid]&&players[uid].name)||"플레이어")}).join("+")||"DM";var dot=(op.getDmUnreadState&&op.getDmUnreadState(ch.channelKey))?dotHtml():"";html.push(`<button type="button" class="pdm-btn ${String(currentKey)===String(ch.channelKey||"")?"on":""}" data-role="channel" data-key="${escHtml(ch.channelKey||"")}"><span class="pdm-label">${escHtml(label)}</span>${dot}</button>`)})}}catch(e){}host.innerHTML=html.join("");var g=host.querySelector("[data-role=global]");if(g)g.onclick=function(){try{if(op.selectGlobalDmChannel)op.selectGlobalDmChannel();if(op.refreshDmChannelButtons)op.refreshDmChannelButtons();switchTab("chat")}catch(e){}};Array.from(host.querySelectorAll("[data-role=player]")).forEach(function(btn){btn.onclick=function(){try{var uid=btn.getAttribute("data-uid")||"";var cur=(op.__DM_CHANNEL_STATE&&Array.isArray(op.__DM_CHANNEL_STATE.selectedParticipantIds)?op.__DM_CHANNEL_STATE.selectedParticipantIds.slice():[]);var i=cur.indexOf(uid);if(i>=0)cur.splice(i,1);else cur.push(uid);if(op.selectDmParticipants)op.selectDmParticipants(cur);if(op.refreshDmChannelButtons)op.refreshDmChannelButtons();switchTab("chat")}catch(e){}};btn.oncontextmenu=function(ev){try{ev.preventDefault();var uid=btn.getAttribute("data-uid")||"";if(!uid||typeof op.getDmButtonAlias!=="function")return false;var current=(op.getDmButtonAlias(uid)||"");var input=window.prompt("표시 이름을 입력해주세요. (최대 8글자, 빈칸 입력 시 초기화)",current);if(input===null)return false;if(typeof op.setDmButtonAlias==="function")op.setDmButtonAlias(uid,input);if(op.refreshDmChannelButtons)op.refreshDmChannelButtons();renderDmBar();return false}catch(e){return false}}});Array.from(host.querySelectorAll("[data-role=channel]")).forEach(function(btn){btn.onclick=function(){try{var key=btn.getAttribute("data-key")||"global";if(op.setCurrentDmChannelKey)op.setCurrentDmChannelKey(key);if(op.refreshDmChannelButtons)op.refreshDmChannelButtons();switchTab("chat")}catch(e){}}})}');
