...
// 추가분만 append
'var needsFullRender=Object.keys(existingKeys).length===0||casualOrderMismatch;if(needsFullRender){window._popoutSuppressAutoscroll=true;c.innerHTML="";arr.forEach(function(item){var el=window.addMsg(item.name,item.text,normalizeMsgType(item.type),item.channel||ch,item.nameColor||"",item.avatar||"",item.time||"",item.fhtml||"",!!item.imageWide,item.imageMeta||null,!!item.hideImageMeta,{key:item.key||item._key||"",uid:item.uid||"",whisperTo:item.whisperTo||"",whisperToName:item.whisperToName||"",whisperToJournal:item.whisperToJournal||""});if(el){el.dataset.popKey=getPopMessageKey(item);attachPopMessageActions(el,normalizeMsgType(item.type),{key:item.key||item._key||"",uid:item.uid||""},item.text||"",item.channel||ch)}});window._popoutSuppressAutoscroll=false;}else{' +
// 기존에 없는 것 추가
'arr.forEach(function(item){var k=getPopMessageKey(item);var oldEl=existingKeys[k];if(!oldEl){var el=window.addMsg(item.name,item.text,normalizeMsgType(item.type),item.channel||ch,item.nameColor||"",item.avatar||"",item.time||"",item.fhtml||"",!!item.imageWide,item.imageMeta||null,!!item.hideImageMeta,{key:item.key||item._key||"",uid:item.uid||"",whisperTo:item.whisperTo||"",whisperToName:item.whisperToName||"",whisperToJournal:item.whisperToJournal||""});if(el){el.dataset.popKey=k;attachPopMessageActions(el,normalizeMsgType(item.type),{key:item.key||item._key||"",uid:item.uid||""},item.text||"",item.channel||ch)}}else{if(oldEl.dataset.popEditing==="1")return;window._popoutSuppressAutoscroll=true;var fresh=window.addMsg(item.name,item.text,normalizeMsgType(item.type),item.channel||ch,item.nameColor||"",item.avatar||"",item.time||"",item.fhtml||"",!!item.imageWide,item.imageMeta||null,!!item.hideImageMeta,{key:item.key||item._key||"",uid:item.uid||"",whisperTo:item.whisperTo||"",whisperToName:item.whisperToName||"",whisperToJournal:item.whisperToJournal||""});window._popoutSuppressAutoscroll=false;if(fresh&&fresh.parentNode===c){fresh.dataset.popKey=k;attachPopMessageActions(fresh,normalizeMsgType(item.type),{key:item.key||item._key||"",uid:item.uid||""},item.text||"",item.channel||ch);c.insertBefore(fresh,oldEl);oldEl.remove();existingKeys[k]=fresh}}});' +
// 새 목록에 없는 것 제거
