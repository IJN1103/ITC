...
L.push('window.renderDmBar=renderDmBar;');
  ...
  L.push('setInterval(function(){try{if(popDmChannelKey==="global"){var op=getOpenerState();if(op&&typeof op.getCurrentDmChannelKey==="function"&&String(op.getCurrentDmChannelKey()||"global")!=="global"&&document.getElementById("pm-chat")&&document.getElementById("pm-chat").children.length===0){popDmChannelKey=String(op.getCurrentDmChannelKey()||"global")||"global"}}catch(e){}renderDmBar();syncDmUnreadDots()},400);');
