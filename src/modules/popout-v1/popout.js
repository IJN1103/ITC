...
L.push('window.renderDmBar=renderDmBar;');
  ...
  L.push('window.setMessages=function(ch,list){ch=ch||"chat";var c=document.getElementById("pm-"+ch)||document.getElementById("pm-chat");if(!c)return;var arr=Array.isArray(list)?list:[];window._popoutMessageSignatures=window._popoutMessageSignatures||{};var sigKey=String(ch||"chat");var sig=makePopMessageSignature(sigKey,arr);if(window._popoutMessageSignatures[sigKey]===sig&&c.dataset.popLastSig===sig)return;var nearBottom=(c.scrollHeight-c.scrollTop-c.clientHeight)<=56;var prevHeight=c.scrollHeight;var prevTop=c.scrollTop;window._popoutSuppressAutoscroll=true;c.innerHTML="";arr.forEach(function(item){window.addMsg(item.name,item.text,normalizeMsgType(item.type),item.channel||ch,item.nameColor||"",item.avatar||"",item.time||"",item.fhtml||"",!!item.imageWide,item.imageMeta||null,!!item.hideImageMeta)});window._popoutSuppressAutoscroll=false;window._popoutMessageSignatures[sigKey]=sig;c.dataset.popLastSig=sig;if(nearBottom||prevHeight===0)c.scrollTop=c.scrollHeight;else c.scrollTop=prevTop+(c.scrollHeight-prevHeight)};');
